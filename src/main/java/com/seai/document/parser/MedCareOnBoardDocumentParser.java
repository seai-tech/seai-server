...
@Component
public class MedCareOnBoardDocumentParser implements DocumentParser {
    ...
    public boolean canParseDocument(List<String> lines) {
        return lines.stream().anyMatch(l-> l.contains("MEDICAL")) && lines.stream().anyMatch(l-> l.contains("CARE"));
    }

    @SneakyThrows
    @Override
    public MarineDocument parseDocument(List<String> lines) {
        String documentType = "MEDICAL CARE ONBOARD";
